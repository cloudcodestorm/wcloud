...
class ChargingRequest
{
    ...
    function __construct($wc_settings)
    {
        $this->set_merchant_id($wc_settings['merchant_id'])
            ->set_merchant_key_id($wc_settings['merchant_key_id'])
            ->set_transaction_type('SALE')
            ->set_ui_mode($wc_settings['ui_mode'])
            ->set_hash_method($wc_settings['hash_method'])
            ->set_currency('INR');

        $this->charging_api_url = $wc_settings['charging_url'];
    ...
    {
        return $this->params[Constants::PARAMETER_UI_MODE];
    }
    ...
    public function validate()
    {
        ...
        if (!Utils::is_valid_parameter($this->params[Constants::PARAMETER_TRANSACTION_AMOUNT])
            ...
            echo "trsanction amount " . $this->params[Constants::PARAMETER_TRANSACTION_AMOUNT];
        ...
        if (($this->params[Constants::PARAMETER_PAYMENT_METHOD] == Constants::PAYMENT_MODE_NET
                || $this->params[Constants::PARAMETER_PAYMENT_METHOD] == Constants::PAYMENT_MODE_EMI
            ) && !Utils::is_valid_parameter($this->params[Constants::PARAMETER_BANK_NAME])
        ) {
            $return_obj["message"] = Constants::INVALID_BANK_NAME;
            return $return_obj;
        }
