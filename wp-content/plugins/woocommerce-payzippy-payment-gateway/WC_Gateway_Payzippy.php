...
require_once('ChargingRequest.php');
require_once('ChargingResponse.php');
    ...
    class WC_PayZippy extends WC_Payment_Gateway
    {
        ...
        function check_payzippy_response()
        {
            ...
            if ($pz_response->get_merchant_transaction_id() != '') {
                ...
                if ($pz_response->validate()) {
                    if ($pz_response->get_transaction_response_code() == Constants::RESPONSE_SUCCESS) {
                        // Payment Successful
                        $message = Constants::PAYMENT_SUCCESS;
                        $class = 'message';
                        $order->payment_complete();
                        $woocommerce->cart->empty_cart();
                    } else if ($pz_response->get_transaction_response_code() == Constants::RESPONSE_PENDING) {
                        // Payment Response Pending.
                        $order->update_status('on-hold');
                        $message = Constants::PAYMENT_ONHOLD;
                        $class = 'message';
                        $woocommerce->cart->empty_cart();
                    } else if ($pz_response->get_transaction_response_code() == Constants::RESPONSE_INITIATED) {
                        // Payment Response Initiated.
                        $order->update_status('pending');
                        $message = Constants::PAYMENT_INITIATED;
                        $class = 'message';
                        $woocommerce->cart->empty_cart();
                    } else {
                        // Payment Failed.
                        ...
                        $class = 'error';
                        $message = Constants::PAYMENT_FAILED;
                    }
                } else {
                    // Hash Validation Failed.
                    $message = Constants::PAYMENT_ILLEGAL;
                    $order->update_status('failed');
        ...
        private function update_order_info(WC_Order $order, ChargingResponse $pz_response)
        {
            ...
            switch ($pz_response->get_payment_method()) {
                case Constants::PAYMENT_MODE_NET:
                    $payment_method = Constants::BANK_NAMES($pz_response->get_bank_name()) . ' ' .
                        Constants::PAYMENT_METHODS($pz_response->get_payment_method());
                    break;
            ...
            <p class="form-row chzn-container-single">
                ...
                <select name="pz_payment_method" id="pz_payment_method" class="chzn-single">
                    ...
                    foreach ($payment_methods as $method)
                        echo '<option value="' . $method . '">' . Constants::PAYMENT_METHODS($method) . '</option>';
                    ?>
                </select>
            </p>
        <?php
        ...
        function template_payment_fields_script($netbanking_banks, $emi_banks)
        {
            ...
            <script type="text/javascript">
                jQuery(function () {
                    var getEMIBanks = function (emi_months) {
                        var options = '';
                        if (emi_months == "3") {
                            options = '<?php echo $emi_banks_options["3"];?>';
                        } else if (emi_months == "6") {
                            options = '<?php echo $emi_banks_options["6"];?>';
                        } else if (emi_months == "9") {
                            options = '<?php echo $emi_banks_options["9"];?>';
                        } else if (emi_months == "12") {
                            options = '<?php echo $emi_banks_options["12"];?>';
                        }
                        return options;
                    };

                    var updatePaymentOptions = function () {
                        var payment_method = jQuery("#pz_payment_method").find("option:selected").val();
                        if (payment_method == "NET") {
                            var options = '<?php echo $netbanking_options;?>';
                            jQuery("#pz_bank_name").html(options);
                            jQuery('.pz_emi_months').hide();
                            jQuery('.pz_bank_fieldset').show();
                        } else if (payment_method == "EMI") {
                            var emi_months = jQuery("#pz_emi_months").find("option:selected").val();
                            jQuery("#pz_bank_name").html(getEMIBanks(emi_months));
                            jQuery('.pz_emi_months').show();
                            jQuery('.pz_bank_fieldset').show();
                        } else {
                            jQuery("#pz_bank_name").html('');
                            jQuery('.pz_emi_months').hide();
                            jQuery('.pz_bank_fieldset').hide();
                        }
                    };

                    updatePaymentOptions();

                    jQuery("#pz_payment_method").change(function () {
                        updatePaymentOptions();
                    });

                    jQuery("#pz_emi_months").change(function () {
                        var emi_months = jQuery("#pz_emi_months").find("option:selected").val();
                        jQuery("#pz_bank_name").html(getEMIBanks(emi_months));
                    });
                });
